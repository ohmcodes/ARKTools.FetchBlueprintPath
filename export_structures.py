...
NamesUnderscored = [
   {'PrimalItemStructure':'PrimalItemStructure_'}
]
...
def createBPPath(asset):
   ...
   # temp
   if 'Structures' not in str(asset.package_path): return ""

   if is_assets_in_exludes(str(asset.package_name)): return ""

   item_name = str(name_components[-1])
...
def export_json(results):
  ...
  with open(base_directory+'blueprints-structures.json', 'w', encoding='utf-8') as json_file:
